...
class _FreeCompositionsPracticeState extends State<FreeCompositionsPractice> {
  ...
  Future<void> _loadPreviousSubmission() async {
    ...
    if (previousSubmission != null) {
      setState(() {
        ...
        gradeColor = (int.parse(grade!) >= 15) ? green : red;
      });
    }
  }
  ...
  void gradeEssay() {
    ...
    final request = GradeFreeCompositionRequest(
      prompt: widget.freeCompositionsPractice.prompt,
    ...
    gradeEssayProvider.gradeFreeComposition(request).then((_) async {
      ...
      if (gradeMatch != null) {
        ...
        setState(() {
          gradeColor = gradeValue >= 15 ? green : red;
        });

        final dbService = Provider.of<DatabaseService>(context, listen: false);
        await dbService.recordEssayResponse('free', widget.compositionType,
